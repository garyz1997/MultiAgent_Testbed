...
public class Resource2Agent extends Agent
{
    protected void setup()
    {
		...
		sd.setType("Resource2Agent"); 
		sd.setName(getName());
		sd.setOwnership("Rpi2");
		dfd.setName(getAID());
		dfd.addServices(sd);
		try {
	...
	private class WaitPingAndRunBehaviour extends CyclicBehaviour {
		...
		public void action() {//run conv2 when receive ping
			...
			if(msg != null){
				...
				if(msg.getPerformative()== ACLMessage.REQUEST){
					...
					if ((content != null) && (content.indexOf("ping") != -1)){
						...
						reply.setContent("running program 2\nSending ping to RA3");
