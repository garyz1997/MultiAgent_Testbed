...
public class Resource1Agent extends Agent
{
    protected void setup()
    {
		...
		sd.setType("Resource1Agent"); 
		sd.setName(getName());
		sd.setOwnership("Rpi1");
		dfd.setName(getAID());
		dfd.addServices(sd);
		try {
	...
	private class WaitPingAndRunBehaviour extends CyclicBehaviour {
		...
		public void action() {//run conv1 when receive ping
			...
			if(msg != null){
				...
				if(msg.getPerformative()== ACLMessage.REQUEST){
					...
					if ((content != null) && (content.indexOf("ping") != -1)){
						...
						reply.setContent("running program 1\nSending ping to RA2");
